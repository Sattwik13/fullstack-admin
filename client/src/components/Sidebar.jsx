...
const Sidebar = ({ user, drawerWidth, isSidebarOpen, setIsSidebarOpen, isNonMobile }) => {
    ...
    return (
        <Box component='nav'>
            {isSidebarOpen && (<Drawer open={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} variant='persistent' anchor='left' sx={{
                width: drawerWidth,
                '& .MuiDrawer-paper': {
                    ...
                    <Box m="1.5rem 2rem 2rem 3rem">
                        <FlexBetween color={theme.palette.secondary.main}>
                            <Box display="flex" alignItems="center" gap="0.5rem">
                                <Typography variant='h4' fontWeight="bold">ANALYSIEREN</Typography>
                            </Box>
                            {!isNonMobile && (
